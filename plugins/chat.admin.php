...
function chat_admin($aseco, $command) {
   ...
   */
    ...
    if($titleid['TitleId']=='SMStorm'){
        ...
        $config_file = 'configs/core/scripts.xml';
   ...
   */
    ...
    if($command['params'][1]!=""){
      ...
      if (array_key_exists($mid, $player->maplist)) {
        $uid = $player->maplist[$mid]['uid'];

        // check map vs. server settings
        $rtn = $aseco->client->query('CheckMapForCurrentServerParams', $player->maplist[$mid]['filename']);
        if($rtn){ //else error 
          $sucess = $aseco->client->query('ChooseNextMap', $player->maplist[$mid]['filename']);
        }
        if($sucess){
          $aseco->client->query('NextMap');
          $aseco->console('{1} [{2}] skips map!', $logtitle, $login);
          
          $message = formatText('{#server}>> {#admin}{1}$z$s {#highlite}{2}$z$s{#admin} changed to {3}!',
                                $chattitle, $admin->nickname,$player->maplist[$mid]['name']);
          $aseco->client->query('ChatSendServerMessage', $aseco->formatColors($message));   
        }             
      } else {
        $message = '{#server}> {#emotic}Map_ID not found - Type {#highlite}/list{#emotic} to see all maps.';
        $aseco->client->query('ChatSendServerMessageToLogin', $aseco->formatColors($message), $login);
      }
    }else{
    // load the next map
   ...
   */
    ...
    $source = 'MX';
    $remotelink = 'https://sm.mania-exchange.com/tracks/download/';
...
/*  } elseif ($command['params'][0] == 'unbanip' && $command['params'][1] != '') {
 ...
 /**
   ...
   */   
    ...
    if(function_exists('scriptSettings_show') && $command['params'][1] == ''){
      scriptSettings_show($admin->login); //Open Scriptsettings Panel  
    }else{  // use standart view
      if($command['params'][1] != ''){     
...
function event_admin($aseco, $answer){
  ...
  // check for /admin force blue command
  elseif ($action >= 8001 && $action <= 8200) {
    $target = $player->playerlist[$action-8001]['login'];

    // log clicked command
    $aseco->console('player {1} clicked command "/admin forceteam  {2} blue"',
                    $player->login, $target);

    // forcespec selected player
    $command = array();
    $command['author'] = $player;
    $command['params'] = 'forceteam ' . $target .' blue';
    chat_admin($aseco, $command);
  ...
  // check for /admin force red command
  elseif ($action >= 8201 && $action <= 8400) {
    $target = $player->playerlist[$action-8201]['login'];

    // log clicked command
    $aseco->console('player {1} clicked command "/admin forceteam  {2} red"',
                    $player->login, $target);

    // forcespec selected player
    $command = array();
    $command['author'] = $player;
    $command['params'] = 'forceteam ' . $target .' red';
    chat_admin($aseco, $command);
